...
class _ResGenerator(nn.Module):
    ...
    def forward(self, input_img):
        ...
        x = (input_image - 0.5) / 0.5
        
        feature = self.encoder(x)
        result = [feature]
        
        # keep outputs format consistent with monodepth2 results
        self.outputs[("gen", 0)] = self.decoder(feature) # last scale feature map
        
        return feature, self.outputs
