...
def parseArgs():
    ...
    parser.add_argument("--scene", type=str, default="Scene20")
    parser.add_argument("--seq", type=str)
    parser.add_argument("--exp", type=str, default="local-0531a")
    return parser.parse_args()

def readCalib(seq):
    lines = []
    with open("data/virtual-kitti/vKitti2/resized_imgs/Camera_0_640x192/camera_640x192.txt", mode="r") as f:
        lines = f.readlines()
...
def warp_right_disp(opt):
    ...
    """
    print("warping {}: {}: {}".format(opt.exp, opt.scene, opt.seq))
    opt.disp_path = "results_DVSO/vKitti2/results/{}/disps".format(opt.exp)
    
    fx, w, h, bl = readCalib(opt.seq)
    disp_left = np.load("{}/{}/{}/disparities_left_pp_640x192.npy".format(opt.disp_path, opt.scene, opt.seq))
    ...
    out_path = "{}/{}/{}/disparities_right_pp_warped.npy".format(opt.disp_path, opt.scene, opt.seq)
...
if __name__ == "__main__":
    opt = parseArgs()
    for seq in ["clone", "fog", "rain", "morning", "overcast", "sunset", "15-deg-left", "15-deg-right", "30-deg-left", "30-deg-right"]:
        opt.seq = seq
        warp_right_disp(opt)
