...
class Solver():
    ...
    def save_depth(self, saved_model, seq, val_iter):
        ...
        if self.opt.save_feat:
            feat_path = os.path.join(self.root_dir, self.saved_models_dir, self.opt.exp, 'feats', seq)
            if not os.path.isdir(feat_path):
                os.makedirs(feat_path)
            if self.opt.post_process:
                raise ValueError("Error: Please do use --post_process for --save_feat")
        else:
        ...
        with torch.no_grad():
            for i, data in enumerate(self.real_val_loaders[seq]):
                ...
                if self.opt.save_feat:
                    curr_batch_size = input_recon.shape[0]
                    for t_id in range(curr_batch_size):
                        t_id_global = (i*self.batch_size)+t_id
                        feat = input_recon.cpu()[t_id].numpy() # (c, h, w)
                        ...
                        feat.save(os.path.join(feat_path, "{:06d}.jpg".format(t_id_global)))
                        if t_id_global % 300 == 0:
