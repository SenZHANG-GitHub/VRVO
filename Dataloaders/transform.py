...
class RandomImgAugment(object):
    ...
    def __call__(self, inputs):
        ...
        # Sen -> Note the difference
        #   torchvision.transoforms.Resize() -> size: (height, width)
        #   PIL.Image.resize() -> size: (width, height)
        ...
        if not self.size == 0:
            # Sen -> In kitti
            #      the depth here is not used! The pred_depth from netT is used for reconstruction instead!
            # Sen -> In vKitti
            #      the depth here is only used for gt supervision! The pred_depth from netT is used for reconstruction instead!
            # Sen -> Thus we don't need to modify the depth values for training here (Except for validation)
            if depth is not None:
        ...
        if self.flip and not (img2 is not None and depth is not None):
            ...
            if img2 is None:
                ...
                # Sen -> seems wrong but right because only flip_prob<0.5 will flip
                if flip_prob < 0.5:
                    img1_ = img1
                    img2_ = img2
            ...
            if depth is not None:
                # Sen -> potential bug here since depth should also be swapped
                #   no influence currently since we don't use depth during training 
